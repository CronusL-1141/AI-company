...
3. **无待办**：主动行动——研究竞品/新技术、组织会议讨论规划（`meeting_create`）、审查代码、优化功能、整理记忆（memo 超阈时 `memory_reconcile_candidates`）。
   - 讨论要**充分论证任务的必要性**：任务不足时讨论方向，**不能为了有事干没事找事干**；讨论产出的目标转为短/中/长期任务上墙。
...
- 派活时统筹并行推进，不等一个完成再开下一个；常驻成员（QA / bug-fixer）保留，只回收临时成员。
