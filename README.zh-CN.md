...
> ⚡ **v1.12.0** — 两个 AI 会话能互相留言，却没人告诉收信方有信来了：对端连发 8 条点名消息、其中一条明确在等回执，在库里躺了半小时无人读。现在 UserPromptSubmit hook 会把「谁在叫你」摆到眼前，调 `channel_read_ack` 清零后下一轮重算为 0、那行提示自行消失。给会话 watcher 带上读者身份武装，新消息可在约 8 秒内**把会话叫醒，全程无需用户输入**——端到端实测两次，其中一次是对端真发的信。过程中收掉四个静默失效缺陷（三个由另一侧 harness 审查本代码时发现），并查出唤醒体系的 `since` 一直在传输中被损坏、watcher 从未真正守望过；新增一条机检禁止两侧共用读者身份——那会让一侧的已读确认抹掉另一侧的信，而两边的测试全都照常通过。
>
