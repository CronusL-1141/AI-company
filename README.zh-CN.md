...
> ⚡ **v1.12.2** — 未读提示引用的是**别人写的文本**，而它的清洗只折叠了空白。那恰好放过了唯一一类以「让你看到的与实际读到的不一致」为存在意义的字符：U+202E 让一行文本视觉反转，U+200B 能在两个可见字符之间藏东西，控制字节原样穿过。**发送者名压根没清洗**，塞进换行就能撑开提示块、伪造出额外的一行。两处都已修掉，注入行现在还明说摘要与发送者名是引用数据而不是指令。这个洞是**审另一侧 harness 的同名 hook 时审出来的**——它那份做对了，我这份没有。同批还有：武装事件 watcher 的会话现在可以声明自己在等哪类信号，因为两个 harness 共用一个项目之后，对端子 agent 的每条进展 memo 都会把本会话唤醒并顺带让 watcher 卸岗。以及：对端自己的 hook 入口脚本终于进了仓库——它们此前只以未跟踪的本地文件存在，那意味着对其行为的任何观测都不构成对仓库代码的证据。
>
