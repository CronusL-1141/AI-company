...
> **提示**：首次启动需要约 30 秒自动配置依赖，仅此一次。后续每次启动 ~100 个 MCP 工具即时可用。
...
<details>
<summary>展开查看全部 ~100 MCP 工具</summary>
...
25 个开箱即用的专业 Agent 模板（23 个基础 + 2 个辩论角色），含推荐引擎，覆盖完整软件工程团队配置：
...
- [x] Dashboard 指挥中心（React 19）+ 3 个可观测性页面
- [x] ~100 MCP 工具
- [x] AWARE 循环记忆系统
- [x] find_skill 三层渐进发现
- [x] task_update API，支持程序化任务管理
- [x] 工作流管道编排（7 种模板 + 自动阶段推进 + 渐进式强制）
- [x] 769 自动化测试（含 28 个跨功能集成测试）
