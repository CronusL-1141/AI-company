...
> ⚡ **v1.12.1** — 那个为「别人叫你时把你叫醒」而武装的 watcher，会被**你自己写的字**轰下岗。它的「子 agent 报进展」信号统计的是项目里任何人写的 memo，包括唤醒者自己刚写的那条；而 watcher 退出会清掉武装心跳——于是写一条 memo 就等于自动卸岗，且全程静默。同批收掉两个同型缺陷：更新若新增依赖，用户可能根本装不上，因为 PEP 668 解释器直接拒绝 `pip install -e .`，而更新脚本只能印一句「依赖若有变动请手动重装」——把判断推给唯一不可能知道答案的人。现在安装与更新不再相信 pip 的返回码，而是逐条验证声明的依赖**能不能 import**，被 PEP 668 拒掉时自动改用 `--break-system-packages` 重试。本版还带来 `channel_wait`：由另一侧 harness 编写并自行提交、经本侧独立审查后合入——它先订阅再补读项目限定的收件箱，并按数据库插入序分页，翻页途中提交的消息因此不会被跳过。
>
