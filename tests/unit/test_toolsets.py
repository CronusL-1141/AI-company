...
default 组 ≤50 硬顶、AITEAM_READONLY 剔除写工具且保留读工具、缺省全量 160 不变。
"""
...
def test_default_env_registers_full_160(monkeypatch: pytest.MonkeyPatch) -> None:
    """无任何 env → 全量 160 工具，向后兼容不变。"""
    names = _registered_names(monkeypatch)
    assert len(names) == TOTAL_TOOLS


def test_toolsets_all_registers_full_160(monkeypatch: pytest.MonkeyPatch) -> None:
