...
> ⚡ **v1.12.2** - The unread line quotes text somebody else wrote, and it was sanitised by collapsing whitespace alone. That leaves exactly the class of character whose only purpose is to make what you see differ from what is there: U+202E reverses a line's visual order, U+200B hides content between two visible characters, and control bytes passed through untouched. The sender name was not sanitised at all, so a newline in it could open the notification block and forge an extra line. Both are fixed, and the injected line now says outright that the excerpt and sender are quoted data rather than instructions. This one was found by reviewing the other harness's copy of the same hook, which had it right. Also: a session that arms the event watcher can now declare which signals it is waiting for, because two harnesses sharing one project meant every progress note written by the peer's subagents woke this side and took the watcher off duty. And the peer's own hook entry scripts are in the repository at last - they had only ever existed as untracked local files, so nothing observed about their behaviour was evidence about the code here.
>
