...
> ⚡ **v1.12.1** - The watcher armed to wake you on someone else's message could be evicted by your own writing. Its "a subagent reported progress" signal counted every memo in the project, including the ones the waking session had just written; a watcher exit clears the armed heartbeat, so writing one memo took the watcher off duty, silently. Two of the same shape were closed with it: an update that adds a dependency could leave the user without it, because PEP 668 interpreters refuse `pip install -e .` and the updater could only print "reinstall if dependencies changed" - handing the question to the one party who cannot answer it. Installs now prove each declared dependency imports rather than trusting pip's exit code, and retry under `--break-system-packages` when the refusal is PEP 668. Also lands `channel_wait`, contributed and committed by the other harness and merged here after independent review: it subscribes before replaying a project-scoped inbox and pages by insertion order, so a message committed during a page turn cannot be skipped.
>
