...
### 3.1 本地墙钟侧 — 需 −8h 平移（21 表 / 45 列 / 320,866 个非空单元）
...
### 3.2 UTC 侧 — 不动（15 表 / 37 列 / 810 个非空单元）
...
变为 `2026-07-28T09:01:24.123456+00:00`（自描述）。

实现方式：不写任何序列化代码。`UtcDateTime` 读出 aware 值 → pydantic / FastAPI 的
`isoformat()` 自动带 offset → 手写 `.isoformat()` 的 74 处调用点同样自动带上。
...
作用于 §3.1 的 45 列 / 320,866 个非空单元。Asia/Shanghai 全年恒定 UTC+8（1991 年后
...
因此脚本内置一条**硬护栏**（`--apply` 时强制，dry-run 时警告）：

> 对每个待平移列，取 `max(值)`；若该列最近 24h 内有写入，则 `max` 必须更接近
> **本地 now** 而非 **UTC now**。一旦更接近 UTC now，说明该列已被新代码写过 UTC 行，
> 整列平移会把这些行推早 8 小时——此时脚本**中止**并报出该列，交人判断。

近 24h 无写入的列（存量冷数据）跳过该检查，只报告，因为无新鲜行可作判据。
...
| 10 | 平移脚本 | dry-run 全表报告可读；护栏在"列已被新代码写过"时确实中止 |
