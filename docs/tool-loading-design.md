...
- **过度工程护栏（用户担忧的显式回应）**：复杂度封顶于此——**不做**衰减曲线/实时计数/后台重算/ML 打分；统计查询失败 → 静默零 alwaysLoad，一切照旧走 ToolSearch（功能纯增益，坏了无损）；名单每次**真正重算**的结果落 events 台账一行（可审计，治理层原则）。
- 与 reconcile 同一架构哲学：**确定性小算法 + 台账已有数据**，零新常驻。

**补记（实测修正，上文"实测 0.1 秒"只在热路径成立）**：端点实测重算 1.58~1.66s（冷启动曾测到 4.6s），而客户端超时原为 2.0s——冷启动会稳定超时并静默降级为零常驻，服务端却照记轮换事件，从台账上看不出失败。两侧各修一处：

- 服务端给重算结果加 **600s TTL 快照**（进程内模块级变量，请求到来时判 TTL 的**惰性刷新**，不起定时器也不起后台任务，"不做后台重算"的护栏仍成立）。命中时直接按 `registered` 过滤返回、**不记事件**——事件量因此反映轮换次数而非调用次数。响应体加 `cached` 与 `computed_at` 两字段。实测命中 1.1~2.5ms。
- 客户端超时放宽到 **3.5s**（仍在 CC 等工具列表的 5s 上限内），可用 `AITEAM_ALWAYSLOAD_TIMEOUT` 覆盖；挂完 best-effort 回报一条 `tool.alwaysload.applied` 事件（`tools` / `elapsed_ms` / `reason` 四值闭集 `""|timeout|http_error|no_api`），与轮换事件成对——一条证明服务端算过，另一条证明客户端真收到了。
